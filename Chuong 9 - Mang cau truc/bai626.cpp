...
int ganGocToaDo(struct TOADO a[], int n);
struct TOADO
{
	int x;
	int y;
...
int main()
{
	...
	int nSize;
	nhap(dcm,nSize);
	cout<<"Diem gan goc toa do nhat: "<<ganGocToaDo(dcm,nSize)<<endl;
}
void nhap(struct TOADO a[], int &n)
{
...
int ganGocToaDo(struct TOADO a[], int n)
{
	int min = a[0].x * a[0].x + a[0].y * a[0].y;
	int index = 0;
	for(int i = 0; i < n; i++)
	{
		if(a[i].x * a[i].x + a[i].y * a[i].y < min)
		{
