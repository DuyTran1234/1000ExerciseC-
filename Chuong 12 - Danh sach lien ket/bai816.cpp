...
node *countPositiveFraction(list l)
{
	node *fractionMem = new node;
	for(node *i = l.pHead; i != NULL; i = i->pNext)
	{
...
int main()
{
	...
	importList(l,n);
	exportList(l);
	cout<<"Phan so duong cuoi cung: "<<countPositiveFraction(l)->data.a<<"/"<<countPositiveFraction(l)->data.b<<endl;
}